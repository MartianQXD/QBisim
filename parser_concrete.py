...
def p_clause_term_define(p):
    ...
    p[0] = Node('BinaryOpProc', str(p[1].stmt)+str(p[2]) +
                str(p[3].stmt), p[2], [p[1], p[3]])
...
def p_term_ifthenelse(p):
    ...
    p[0] = Node('IfThenElse', str(p[1])+" "+str(p[2].stmt)+" " +
                str(p[3])+" "+str(p[4].stmt), p[1], [p[2], p[4]])
...
def p_action_binaryopact(p):
    ...
    p[0] = Node('BinaryOpAct', str(p[1].stmt)+str(p[2]) +
                str(p[3].stmt), p[2], [p[1], p[3]])
...
def p_expression_binaryopexpr(p):
    ...
    p[0] = Node('BinaryOpExpr', str(p[1].stmt)+str(p[2]) +
                str(p[3].stmt), p[2], [p[1], p[3]])
