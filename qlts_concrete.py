...
def ast_trans2_qLTS(tree,current_state,statement,evaluation):
	...
	#新添加运算符Next，帮助Parallel的描述
		...
		state = 0
		ast_trans2_qLTS(tree.children[0],state,tree.children[0].stmt,evaluation)
		state = 0
		ast_trans2_qLTS(tree.children[1],state,tree.children[1].stmt,evaluation)
		return
	elif tree.type == 'BinaryOpProc':
		if tree.leaf == '+':
			if tree.children:
				#print("Branching")
				ast_trans2_qLTS(tree.children[0],current_state,tree.children[0].stmt,evaluation)
				ast_trans2_qLTS(tree.children[1],current_state,tree.children[1].stmt,evaluation)
			else:
			...
			if tree.children:
				...
				current_label = ast_trans2_qLTS(tree.children[0],current_state,tree.children[1].stmt,evaluation)
				if tree.children[0].type == 'Measurement':
					state = state+1
					transition.append((current_state,state,tau,"measurement",evaluation))
					...
					measurement = temp_method_for_building_superoperator(measurement,qbit)
					n = 0
					for m in measurement:
						current_superoperator = snapshot[current_state]['density operator']
						new_superoperator = np.dot(np.dot(m,current_superoperator),m)
						new_superoperator = np.around(new_superoperator,decimals=3)
						prob = trace(new_superoperator)
						#print("Trace: ",trace)
						if prob == 0:
							print("Can not normalize the superoperator. Trace is 0.")
						else:
							...
							transition.append((current_state,state,"p"+str(int(n))+"="+str(prob),'measurement'))
							next_label = ast_trans2_qLTS(tree.children[1],state,tree.children[1].stmt,new_evaluation)
						n = n+1
				elif tree.children[0].type == 'Superoperator':
					state += 1
					transition.append((current_state,state,tau,current_label[1],evaluation))
					add_slilent_SN(current_state,statement,'silent')
					current_state = state
					text = tree.children[0].leaf
					...
					for m in mat:
						new_operator = new_operator+np.dot(np.dot(m,current_operator),m.conjugate().transpose())
						new_operator = np.around(new_operator,decimals=3)
					prob = trace(new_operator)
					state += 1
					new_snapshot = { 'term': tree.children[1].stmt, 'probability': prob,
									 'kraus operator': mat,
						 			 ...
						 			 'action name' : text}
					snapshot.append(new_snapshot)
					transition.append((current_state,state,"p="+str(prob),current_label[1],evaluation))
					next_label = ast_trans2_qLTS(tree.children[1],state,tree.children[1].stmt,evaluation)
					...
					for name in para:
						if name in evaluation.keys():
							qubit.update({name : number2qubit(evaluation[name])})
					n = len(qubit[para[2]])
					for i in range(n):
					...
					for i in range(2**not_matched_len):
						...
						new_snapshot = { 'term': tree.children[1].stmt, 'probability': 0.5,
									 'kraus operator': mat,
						 			 ...
						 			 'action name' : process}
						snapshot.append(new_snapshot)
						transition.append((current_state,state,"p="+str(1/(2**not_matched_len)),current_label[1],new_evaluation))
						next_label = ast_trans2_qLTS(tree.children[1],state,tree.children[1].stmt,new_evaluation)
					...
					state += 1
					transition.append((current_state,state,current_label[0],current_label[1],evaluation.copy()))
					next_label = ast_trans2_qLTS(tree.children[1],state,tree.children[1].stmt,evaluation)
			...
			if tree.children:
				...
				qLTS.append((get_process_name(tree.children[0].leaf),tree.children[1]))
				...
				state = 0
				#pprint.pprint(snapshot)
				ast_trans2_qLTS(tree.children[1],current_state,tree.children[1].stmt,regx_val.copy())
		...
		if tree.children:
			...
			if tree.children[0].children[0].type=='CMP' and tree.children[0].children[1].type=='CMP':
				evaluation.update(cmp_function2(tree.children[0].children[0],evaluation))
				evaluation.update(cmp_function2(tree.children[0].children[1],evaluation))
...
def parallelism(collection,current_state,tree,evaluation):
	if tree.leaf=="||":
		...
		for process_name in collection:
			qlts_index = [x for x in range(len(qLTS)) if qLTS[x][0] == process_name]
			if len(qlts_index)>1:
...
def have_matched(acts):
	input_act_pos = [t for t in range(len(acts)) if contains_act_(acts[t],'?')==True]
	output_act_pos = [t for t in range(len(acts)) if contains_act_(acts[t],'!')==True]
	...
	if have_matched(acts):
		...
		for i in input_act_pos:
			for o in output_act_pos:
				...
				for action1 in acts[i]:
					for action2 in acts[o]:
						new_temp_components = update_components2(i,o,action1,action2,temp_components)
						if new_temp_components!=None:
							state = state+1
							new_evaluation = evaluation
							new_evaluation = update_variable(action1,action2,evaluation)
							transition.append((current_state,state,tau,'matched'))
							add_slilent_SN(current_state,gen_parallel_stmt(new_temp_components),'matched')
							parallelism_trans2_qLTS(new_temp_components,state,new_evaluation)
		input_act_pos = [t for t in range(len(acts)) if contains_act_(acts[t],'.?')==True]
		output_act_pos = [t for t in range(len(acts)) if contains_act_(acts[t],'.!')==True]
		for i in input_act_pos:
			for o in output_act_pos:
		...
		for t in range(len(acts)):
			b = contains_act_(acts[t],'?') or contains_act_(acts[t],'!') or contains_act_(acts[t],'.?') or contains_act_(acts[t],'.!') or None in acts[t]
			if b==False:
				act_pos.append(t)
		if len(act_pos)==0:
	...
	if len(act_pos)>0:
		...
		if len(acts[a])>0:
			...
			if action.children[0].type == 'Silent':
				new_temp_components = update_components1(a,action,temp_components)
				state = state+1
				transition.append((current_state,state,tau,'silent'))
				add_slilent_SN(current_state,gen_parallel_stmt(new_temp_components),'silent')
				parallelism_trans2_qLTS(new_temp_components,state,evaluation)
				...
				new_snapshot = { 'term': gen_parallel_stmt(new_temp_components), 'probability': prob,
								 'kraus operator': mat,
					 			 ...
					 			 'action': text }
				snapshot.append(new_snapshot)
				transition.append((current_state,state,"p="+str(prob),'silent'))
				parallelism_trans2_qLTS(new_temp_components,state,evaluation)
				...
				evaluation.update(dic)
				state += 1
				transition.append((current_state,state,tau,'silent',evaluation))
				add_slilent_SN(current_state,statement,'silent')
				parallelism_trans2_qLTS(new_temp_components,state,evaluation)
			else:
				...
				if action.children[0].leaf == '!' or action.children[0].leaf == '.!':
					...
					if func.type == 'CMP':
						...
						new_stmt = action.children[0].children[0].stmt+action.children[0].leaf+result
					elif func.type == 'Sub':
				...
				new_temp_components = update_components1(a,action,temp_components)
				state = state+1
				transition.append((current_state,state,new_stmt,action.children[0].leaf.lower()))
				add_slilent_SN(current_state,gen_parallel_stmt(new_temp_components),action.children[0].leaf)
				parallelism_trans2_qLTS(new_temp_components,state,evaluation)
	return
